...
], function($, _, AnycookAPI, Search, User, searchView, stringTools, tags, time,
            ingredientRowTemplate) {
    'use strict';
    // alle Filter
    return {
        ...
        reset: function() {
            ...
            $('#userfilter').hide();
            $('#filter_main').show().css(
                {paddingBottom: '20px', height: 'auto'});
            $('#filter_main *').not(
                'ul.kategorie_filter, #userfilter, #userfilter *, label .active').show().css(
                'opacity', 1);
            ...
            //blockFilter(false);
            this.handleRadios(
                $('#filter_table .label_chefhats, #filter_table .label_muffins'));
        ...
        ingredientListClick: function() {
            if (!$('#filter_main').is('.blocked')) {
                ...
                if ($input.length > 0) {
                    ...
                    $li.append(
                        '<input type="text" /><div class="close"></div>');
                    ...
                    $input
                        ...
                        .autocomplete({
                            source: function(req, resp) {
                                ...
                                $('#ingredient_list .ingredient').each(
                                    function() {
                                        excludedIngredients.push(
                                            $(this).text());
                                    });

                                AnycookAPI.autocomplete.ingredient(term,
                                    excludedIngredients, function(data) {
                            ...
                            select: function(event, ui) {
                                ...
                                $('#ingredient_list input').autocomplete(
                                    'destroy');
        ...
        setUserfilter: function(userId) {
            ...
            AnycookAPI.user(userId, function(user) {
                ...
                if ($('#userfilter').css('display') === 'none') {
                    $('#userfilter').css(
                        {display: 'block', opacity: 0, height: 0}).animate(
                        {height: 50}, {
                            duration: 300,
                            complete: function() {
        ...
        removeUserfilter: function() {
            $('#userfilter').animate({opacity: 0}, {
                duration: 400, complete: function() {
                    $(this).animate({height: 0}, {
                        duration: 300, complete: function() {
                            $(this).css(
                                {display: 'none', opacity: 1, height: 50});
        ...
        setFromRecipe: function(recipe) {
            ...
            $('#time_form .time_text_end').text(
                time.fillStd(recipe.time.std) + ' : ' + time.fillMin(
                    recipe.time.min) + ' h');
        ...
        makeIngredientHeaderForRecipe: function(personNum) {
            var $zutatHead = $('#zutat_head').empty().append(
            ...
            $personsForm.children('.down').click(
                $.proxy(this.personsDown, this));
        ...
        multiplyIngredients: function(perscount, recipe) {
            ...
            $('#ingredient_list .amount, .ingredients .amount').each(
                function(i) {
                    ...
                    var newValue = stringTools.getNumbersFromString(amount,
                        perscount);
