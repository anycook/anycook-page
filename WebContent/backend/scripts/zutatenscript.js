function loadZutaten(json){
	...
	for(var i in json){
		...
		htmlstring += "<div class=\"zutatenname\">"+json[i].name+"</div>" +
				"<div class=\"zutatensingular\">Singular: "+singular+"</div>" +
						"<div class=\"zutatengerichte\">Gerichte: "+
		json[i].gerichte+"</div><div class=\"zutatenchildren\">Childs: "+json[i].childs+"</div><div class=\"setparent\">set parent</div>" +
				"<div class=\"setsingular\">set singular</div>" +
				"<ul></ul></li>";
		$("#zutatenlist").append(htmlstring);
	}
	
	$(".setparent").live("click", setParent);
	$(".setsingular").live("click", setSingular);
...
function loadChilds(json, ul){
	for(var i in json){
		...
		htmlstring += "<div class=\"zutatenname\">"+json[i].name+"</div><div class=\"zutatensingular\">Singular: "+singular+"</div><div class=\"zutatengerichte\">Gerichte: "+
		json[i].gerichte+"</div><div class=\"zutatenchildren\">Childs: "+json[i].childs+"</div><div class=\"setparent\">set parent</div>" +
			"<div class=\"setsingular\">set singular</div>" +
				"<ul></ul></li>";
...
function setParent(event){
	$("#confirm_form").unbind("submit", singularConfirmed);
	$("#confirm_form").submit(parentConfirmed);
	var target = $(event.target);
	var zutat = target.siblings(".zutatenname").first().text();
	$("#confirm_text").html("Namen der neuen Parent-Zutat für \"<span id=\"selected_zutat\"></span>\" eingeben:");
...
function setSingular(event){
	$("#confirm_form").unbind("submit", parentConfirmed);
	$("#confirm_form").submit(singularConfirmed);
	var target = $(event.target);
	var zutat = target.siblings(".zutatenname").first().text();
	$("#confirm_text").html("Singular für \"<span id=\"selected_zutat\"></span>\" eingeben:");
	$("#selected_zutat").text(zutat);	
	if($("#confirm").css("display")=="none"){
		var confirmheight = $("#confirm").css("height");
		confirmheight = Number(confirmheight.substring(0, confirmheight.length-2))+30+30;
...
function singularConfirmed(){
	var zutat = $("#selected_zutat").text();
	var singular = $("#confirm_input").val();
	$.ajax({
		url:"/anycook/EditZutaten",
		data:"todo=setsingular&zutat="+zutat+"&newname="+singular,
		success:function(){
			$.ajax({
				url:"/anycook/GetZutaten",
				dataType:"json",
